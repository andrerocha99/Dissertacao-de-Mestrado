...
for index, row in ataques_2021.iterrows():
    ...
    index_time = np.where(np.array(date1)==datetime.combine(date(year,month,day),time(Data_conv_rain)))[0][0]
    print(index_time)
    ...
    if np.ma.is_masked(rain_val):
        ...
        wind_val = np.mean(data1.variables['pr'][index_time-0:index_time+1,min_index_lat-4:min_index_lat+5,min_index_lon-4:min_index_lon+5])
