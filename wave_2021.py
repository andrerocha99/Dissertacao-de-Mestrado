...
for index, row in ataques_2021.iterrows():
    ...
    index_time = np.where(np.array(date1)==datetime.combine(date(year,month,day),time(Data_conv_wave)))[0][0]
    print(index_time)
    wave_val = data1.variables['VHM0'][index_time][min_index_lat][min_index_lon]
    
    if np.ma.is_masked(wave_val):
        print(index)
        print(np.mean(data1.variables['VHM0'][index_time-0:index_time+1,min_index_lat-4:min_index_lat+5,min_index_lon-4:min_index_lon+5]))
        wind_val = np.mean(data1.variables['VHM0'][index_time-0:index_time+1,min_index_lat-4:min_index_lat+5,min_index_lon-4:min_index_lon+5])
...
ataques_new_2021['onda']=wave

ataques_new_2021.to_excel('novo_com_wave_21.xlsx')
