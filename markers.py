...
ataques = pd.read_excel(r'C:\Users\35192\Desktop\Tese\Criar Base de Dados\BD apos o pre-processamento\BD Finais\Base_Dados_Final.xlsx')
...
ataques = ataques[ataques.Ano.isin([kik_ano,lo_ano])]  #quando se quer apenas para 1 ano especifico
#ataques = ataques[ataques.Ano>jul_ano]
...
Tipo_ataque = list(ataques['Classificação_Ataque'])
...
Estado_Costeiro = list(ataques['Estado_Costeiro'])
Tipo_Armamento = list(ataques['Armamento'])
...
for lat,lon,ta,dh,nip,nc,arm,ec, colored in zip(lat,lon,Tipo_ataque,Data_hora,Nivel_Protecao,Numero_Criminosos,Tipo_Armamento,Estado_Costeiro,col_ata):
    fg.add_child(folium.Marker(location=[lat,lon],
                                popup=folium.Popup("<b>Classificação Ataque : </b>"+str(ta) + "<br> <b>Data Hora : </b>"+str(dh) +
                                                  "<br> <b>Nível de Proteção : </b> "+
                                                    str(nip) +"<br><b>Nº de Criminosos : </b>" +
                                                    str(nc) +"<br><b>Tipo de Armamento : </b>" +
                                                    str(arm) + "<br><b>Estado Costeiro : </b>" + 
                                                    str(ec),max_width=300,min_width=0),
...
for i, box in enumerate(grid):
    ...
    if np.isnan(criminosos):
        criminosos = 0
    perigosidade = amostra_box.Perigosidade.mean()
    if np.isnan(perigosidade):
        perigosidade = 0
    texto = 'Média de criminosos: {}; <br>Média de perigosidade: {}; <br>N.º de ataques registados: {}; <br>Nível de ataque médio: {};'.format(str(criminosos)
                                                                                                              ,str(perigosidade),
                                                                                                              str(n_ataques),
                                                                                                              str(nivel),
                                                                                                             )
    popup = folium.Popup(texto, max_width=300,min_width=0)
    popups.append(popup)
...
max_nivel = 1.5 #permite alterar o quão vermelho e o quanto queremos dar destaque à pintura, se diminuir muito o valor as cores ficam muito parecidas
...
title = 'Marcadores com Classificação de Ataque desde {0} até {1}'.format(in_ano,kik_ano)
title_html = '''
...
m.save('Marcadores 2019 a 2020.html')
